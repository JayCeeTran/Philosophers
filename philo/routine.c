...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/25 13:36:36 by jtran             #+#    #+#             */
/*   Updated: 2025/08/25 13:40:20 by jtran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	round_1(t_phil *phil, t_data *data, long time)
{
	if (phil->id % 2 == 0)
	{
		if (data->philos % 2 == 1)
		{
			...
			while (!data->dead && get_time(data->start_time)
				- time < data->ms_eat * 0.8 * 2)
				usleep(500);
		}
		else
		{
	...
	{
		time = get_time(data->start_time);
		while (!data->dead && get_time(data->start_time) - time < data->ms_eat
			* 0.5)
			usleep(500);
	}
}
...
void	other_rounds(t_data *data, long time, long think_time)
{
	...
	if (think_time > 50)
	{
		while (!data->dead && get_time(data->start_time) - time < think_time
			* 0.75)
			usleep(500);
	}
}
